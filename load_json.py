...
def load_json_data():
    ...
    folder_name = input("Enter the folder name where the JSON file is located: ") #inorderd to work with json file ... importing json and using pandas
    file_name = 'sbatchu_adoptions.json'
    file_path = os.path.join('OneDrive', 'Desktop', folder_name, file_name)# reading the file,loading the file into data variable 
    try:
...
def csv_files(data, filename):
    df = pd.DataFrame(data) #5 files ....data frames are converted into csv files by using this csv_files function
    df.to_csv(filename, index=False)

def contacts_file(data):
    contacts = []   #seprating  contacts ...initializing the contacts for a particular record this is the function for 
    for item in data:
        if 'contacts' in item:
...
def universities_file(data):
    universities = [] # we are initializinfg universties function....
    for item in data :
        universities.append(item['university']) # appending
    csv_files(universities, 'universities.csv') #data frames into csv 
